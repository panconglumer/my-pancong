...
const Popular = () => {
  ...
  return (
    <div className=' p-5 sm:p-10 lg:px-28' id='popular'>
        <p className='md:text-[32px] text-[28px] font-bold text-center'>Popular Dishes</p>
         ...
         <div className='cards py-4 '>
          {allFoods.map(({ id ,image01 , title , quantity , category}) => {
            ...
            toast.info("Item Removed from cart!" , { position: toast.POSITION.TOP_LEFT }) 
             ...
             return (
                ...
                <span class="animate-ping absolute  h-4 w-4 top-0 right-0  z-10 rounded-full bg-red-400 opacity-75"></span>
                     ...
                     <div className='flex text-sm text-center py-2 font-bold justify-between items-center w-full p-2 '><p className=' p-1 rounded-lg text-slate-800'></p> {cartItems && !cartItems.some((item) => item.id === id ) ? (<span onClick={() => {
                        addItem({ id , title ,image01 , quantity ,category})
