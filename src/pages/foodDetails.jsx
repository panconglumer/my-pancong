...
const FoodDetails = () => {
  ...
  const foodDetails = products.find((item) => {
   ...
   const { image01 , image02, image03, quantity , rating  , title , desc ,  category } = foodDetails
   ...
   const [ showDescription  , setShowDescription] = useState('desc')
    ...
    const addItem = () => {
      addToCart({ id , title ,image01 , quantity , category})
   ...
   const remove = () => {
    ...
    const addNewItem = () => {
      ...
      if(!existingItem){
        toast.success(`${title} added to cart!`, {
          position: toast.POSITION.TOP_RIGHT })
        return  addToCart({ id  , title ,image01 , quantity , category})
      } else{
   ...
   <CommonSection  title={title}></CommonSection>
      ...
      <div className='flex flex-col gap-y-2 '>
        ...
        <p className='flex gap-x-1 items-center'>{ [...Array(5)].map((_ , i) => (
         ...
         showDescription === "desc" ? (<div className='flex flex-col gap-x-1 py-1 mb-1 gap-y-2'>
          ...
          <div>
              <p className='font-semibold my-1  text-md'>Untuk request toping & rasa langsung order via wa</p>
        <p className='font-semibold my-1  text-md'>Select Quantity</p>

        <div className='flex gap-y-1 sm:gap-y-0  gap-x-1 text-xs items-center'>

        <span onClick={() => setSize("small")} className={`rounded-lg p-2 border border-slate-300 shadow-md cursor-pointer  ${showSize === 'small' ? "border-slate-600 border" : "" }`}>1</span>
        <span  onClick={() => setSize("medium")} className={`rounded-lg p-2 border border-slate-300 shadow-md cursor-pointer ${showSize === 'medium' ? "border-slate-600 border" : "" } `}>2</span>
        <span   onClick={() => setSize("large")} className={`rounded-lg p-2  border border-slate-300 shadow-md cursor-pointer ${showSize === 'large' ? "border-slate-600 border" : "" } `}>3</span>
        <span   onClick={() => setSize("xlarge")} className={`rounded-lg p-2  border border-slate-300 shadow-md cursor-pointer ${showSize === 'xlarge' ? "border-slate-600 border" : "" } `}>4</span>
    ...
    relatedProducts.map(({ id, price ,image01 , title , quantity , category})=> {
      ...
      <span class="animate-ping absolute  h-4 w-4 top-0 right-0  z-10 rounded-full bg-red-400 opacity-75"></span>
        ...
        <div className='absolute top-0 text-slate-900 right-0 bg-gradient-to-br from-slate-50 to-red-300 font-semibold rounded-tr-lg rounded-bl-lg text-xs overflow-hidden p-1'></div>
       </motion.div>
      </div>
      )
    }).slice(0,4)
